...
- (void)_setupViewsWithType:(EMMessageType)aType
{
    ...
    UITapGestureRecognizer *extImageGesture = [[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
        if (weakSelf.avatarViewTapBlock) {
            weakSelf.avatarViewTapBlock();
        };
    }];
    [self.avatarView addGestureRecognizer:extImageGesture];
...
- (void)setModel:(EMMessageModel *)model
{
    ...
    if (model.direction == EMMessageDirectionSend) {
        ...
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:[CXClientModel instance].avatar]];
    } else {
//        self.nameLabel.text = model.emModel.from;
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:_friendModel.avatar]];
