...
class ISVGAParser(private val context: Context) {
    ...
    fun decodeFromURL(url: URL, callback: ParseCompletion): (() -> Unit)? {
        if (SvgaUtils.isCached(buildCacheKey(url))) {
            threadPoolExecutor.execute {
                val entity = SvgaUtils.getMovieEntity(buildCacheKey(url))
                if (entity != null && entity is MovieEntity) {
                    val videoItem = SVGAVideoEntity(entity, File(buildCacheKey(url)))
                    videoItem.prepare {
                ...
                readAsBytes(inputStream)?.let { bytes ->
                    if (bytes.size > 4 && bytes[0].toInt() == 80 && bytes[1].toInt() == 75 && bytes[2].toInt() == 3 && bytes[3].toInt() == 4) {
                        ...
                        inflate(bytes)?.let {
                            ...
                            val videoItem = SVGAVideoEntity(movieEntity, File(cacheKey))
                            videoItem.prepare {
                                threadPoolExecutor.execute {
                                    SvgaUtils.saveMovieEntity(cacheKey, movieEntity)
                                }
                                this.invokeCompleteCallback(videoItem, callback)
                            }
                        }
