...
public class LevelImageSpan extends ImageSpan {
    ...
    public LevelImageSpan(Drawable d, Context context) {
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        if (mVerticalAlignment == ALIGN_CENTER) {
            ...
            paint.setTextSize(ResUtils.sp2px(mContext, 9));
            Paint.FontMetrics fontMetrics = paint.getFontMetrics();
            float baseline = (top + bottom - fontMetrics.top - fontMetrics.bottom) / 2;
            float dx = x + b.getBounds().right - paint.measureText(text, start, end) - ResUtils.dp2px(mContext, 4);
