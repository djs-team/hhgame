...
+ (DDAudioLRC *)parser:(NSString *)lrc
{
    ...
    NSArray *tempArray = [self sortedLRCUnits:units];
    for (int i = 0; i < tempArray.count; i++) {
        DDAudioLRCUnit *model = tempArray[i];
        if (i < tempArray.count - 1) {
            DDAudioLRCUnit *model2 = tempArray[i+1];
            model.endTime = model2.startTime;
            model.next_lrc = model2.lrc;
            model.lrc_row = i;
        } else {
            model.endTime = model.startTime + 0.010;
            model.next_lrc = @"";
        }
    }
...
- (NSRange)linesAtTimeSecondFrom:(NSTimeInterval)from to:(NSTimeInterval)to
{
    ...
    [self.units enumerateObjectsUsingBlock:^(DDAudioLRCUnit *obj, NSUInteger idx, BOOL *stop) {
        if (obj.startTime >= start && obj.startTime <= end) {
            [indexSets addIndex:idx];
        }
        if (obj.startTime > end) {
            *stop = YES;
        }
    }];
...
- (void)setSecString:(NSString *)secString
{
    ...
    self->_startTime = time * 1000;
}

- (NSString *)description
{
    return [NSString stringWithFormat:@"%@, timeSecond:%.0f , LRC:%@",[super description],self.startTime,self.lrc];
}
@end
