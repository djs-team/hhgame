 ...
 */
package com.deepsea.mua.im.adapter;
...
import com.deepsea.mua.im.widget.presenter.EaseChangewxPresenter;
import com.deepsea.mua.im.widget.presenter.EaseChatBigExpressionPresenter;
import com.deepsea.mua.im.widget.presenter.EaseChatFilePresenter;
import com.deepsea.mua.im.widget.presenter.EaseChatImagePresenter;
import com.deepsea.mua.im.widget.presenter.EaseChatRowPresenter;
import com.deepsea.mua.im.widget.presenter.EaseChatTextPresenter;
import com.deepsea.mua.im.widget.presenter.EaseChatVoicePresenter;
import com.deepsea.mua.im.widget.presenter.EaseGiftPresenter;
...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int MESSAGE_TYPE_SENT_WXCHANGE = 14;
	private static final int MESSAGE_TYPE_RECV_WXCHANGE = 15;
	private static final int MESSAGE_TYPE_SENT_GIFT = 16;
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			String type = message.getStringAttribute("type", null);
			if (type != null && type.equals("wx")) {
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_WXCHANGE : MESSAGE_TYPE_SENT_WXCHANGE;
			} else if (type!=null&&type.equals("gift")){
	...
	protected EaseChatRowPresenter createChatRowPresenter(EMMessage message, int position) {
        ...
        case TXT:
			String type = message.getStringAttribute("type", null);
			if (type != null && type.equals("wx")) {
				presenter = new EaseChangewxPresenter();

			}else if (type != null && type.equals("gift")){
				presenter=new EaseGiftPresenter();
			}else if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
