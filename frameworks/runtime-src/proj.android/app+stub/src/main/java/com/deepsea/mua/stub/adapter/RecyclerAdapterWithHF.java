...
public class RecyclerAdapterWithHF extends RecyclerView.Adapter<ViewHolder> {
    ...
    private void prepareHeaderFooter(HeaderFooterViewHolder vh, View view) {
        ...
        if (mManagerType == TYPE_MANAGER_GRID && mRecycleView != null) {
            final GridLayoutManager layoutManager = (GridLayoutManager) mRecycleView.getLayoutManager();
    ...
    public void removeAllFooterView() {
        if (mFooters.size() > 0) {
            int footerViewSize = mFooters.size();
            for (int i = 0; i < footerViewSize; i++) {
                View childView = mFooters.get(i);
                removeFooter(childView);
            }
        }
    }
    ...
    public void showFooterView(boolean isShow) {
        if (mFooters.size() > 0) {
            ...
            for (int i = 0; i < footerViewSize; i++) {
                if (isShow) {
                    mFooters.get(i).setVisibility(View.VISIBLE);
                } else {
                    mFooters.get(i).setVisibility(View.INVISIBLE);
                }
            }
        }
    }
    public void showHeaderView(boolean isShow) {
        if (mHeaders.size() > 0) {
            int footerViewSize = mHeaders.size();
            for (int i = 0; i < footerViewSize; i++) {
                if (isShow) {
                    mHeaders.get(i).setVisibility(View.VISIBLE);
                } else {
                    mHeaders.get(i).setVisibility(View.INVISIBLE);
                }
            }
        }
    }
