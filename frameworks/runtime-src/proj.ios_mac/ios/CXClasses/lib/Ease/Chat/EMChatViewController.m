...
//    }
    //礼物
    CXChatRoomGIftView *giftView = [[[NSBundle mainBundle] loadNibNamed:@"CXChatRoomGIftView" owner:self options:nil] lastObject];
    [self getGiftListData];
    kWeakSelf
    giftView.rechargeAction = ^{
        CXRechargeViewController * vc = [CXRechargeViewController new];
        [weakSelf.navigationController pushViewController:vc animated:YES];
    };
    giftView.sendGiftAction = ^(CXLiveRoomGiftModel * _Nonnull model) {
...
- (void)getGiftListData {
    ...
    [CXHTTPRequest POSTWithURL:@"/index.php/Api/Gift/getList" parameters:param callback:^(id responseObject, BOOL isCache, NSError *error) {
        if (!error) {
            NSArray *giftArrays = [NSArray modelArrayWithClass:[CXLiveRoomGiftModel class] json:responseObject[@"data"][@"gift_info"]];
            [weakSelf.chatBar.giftView reloadGiftData:giftArrays];
        }
    }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cellString length] > 0) {
        ...
        if (model.type == EMMessageTypeText) {
            EMMsgTextBubbleView *bubble = (EMMsgTextBubbleView *)cell.bubbleView;
            kWeakSelf
            bubble.extImageViewTapBlock = ^{
                [weakSelf playGiftAnimation:model.emModel.ext[@"animation"]];
            };
        }
        
        cell.avatarViewTapBlock = ^{
...
//发送礼物消息
- (void)sendGiftWithGiftModel:(CXLiveRoomGiftModel *)giftModel {
    
    NSString *signature = [CocoaSecurity md5:[CXClientModel instance].token].hexLower;
    NSDictionary *param = @{
        @"signature": signature,
        @"touid": self.conversationModel.emModel.conversationId,
        @"gift_id": giftModel.gift_id.stringValue,
    };
    kWeakSelf
    [CXHTTPRequest POSTWithURL:@"/index.php/Api/Friend/sendGift" parameters:param callback:^(id responseObject, BOOL isCache, NSError *error) {
        if (!error) {
            NSDictionary *ext = @{
                                  @"type": @"gift",
                                  @"gift_url" : giftModel.gift_image,
                                  @"animation" : giftModel.animation,
                                  };
...
#pragma mark - Gift
- (void)playGiftAnimation:(NSString *)animation {
...
- (CXChatRoomPlayGiftView *)playGiftView {
    if (!_playGiftView) {
        _playGiftView = [[[NSBundle mainBundle] loadNibNamed:@"CXChatRoomPlayGiftView" owner:self options:nil] lastObject];
        _playGiftView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    }
    
    return _playGiftView;
}
