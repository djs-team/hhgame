 ...
 */
public class MicManageAdapter extends FragmentPagerAdapter {

    private BaseFragment[] fragments = null;
    private final String[] RANK_TITLES;


    public MicManageAdapter(FragmentManager fm, OnManageListener mListener, List<MicroOrder> micMan, String sex, int pageNum, int canSelectMicroNum, int onMicroCost) {
        super(fm);
        RANK_TITLES = new String[]{"已申请", "房间内", "最近活跃", "好友"};
        fragments = new BaseFragment[]{SortApplyMicFragment.newInstance(micMan, sex, mListener), SortVisitorInRoomForManyFragment.newInstance(pageNum, canSelectMicroNum, onMicroCost, mListener), SortRecentActiveForManyFragment.newInstance(mListener, canSelectMicroNum, onMicroCost,sex), SortFriendsFragment.newInstance(mListener, canSelectMicroNum, onMicroCost,sex)};
    }
    ...
    public void setApplyData(List<MicroOrder> data) {
        ((SortApplyMicFragment) fragments[0]).setData(data);
    }
