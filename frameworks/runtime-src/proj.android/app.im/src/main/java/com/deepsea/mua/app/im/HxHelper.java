...
import com.deepsea.mua.app.im.db.DemoDBManager;
import com.deepsea.mua.app.im.db.InviteMessgeDao;
import com.deepsea.mua.app.im.db.UserDao;
import com.deepsea.mua.app.im.domain.InviteMessage;
import com.deepsea.mua.app.im.domain.InviteMessage.InviteMessageStatus;
import com.deepsea.mua.app.im.domain.RobotUser;
import com.deepsea.mua.app.im.parse.UserProfileManager;
import com.deepsea.mua.app.im.receiver.HeadsetReceiver;
import com.deepsea.mua.app.im.ui.ChatActivity;

import com.deepsea.mua.app.im.utils.PreferenceManager;
...
import com.deepsea.mua.im.domain.EaseEmojiconGroupEntity;
import com.deepsea.mua.im.domain.EaseUser;
import com.deepsea.mua.im.model.EaseAtMessageHelper;
import com.deepsea.mua.im.model.EaseNotifier;
...
public class HxHelper {
    ...
    public static final String ACTION_LOGIN_ANOTHER = "com.deepsea.mua.login_another";
    ...
    private HxSettingModel mSettingModel = null;
    ...
    public void init() {
        HyphenateClient.getInstance().setHyphenateDelegate(new IHyphenateClient.HyphenateDelegate() {
            @Override
            public void init(Context context) {
                HxHelper.this.init(context);
            }
            ...
            public void saveContact(EaseUser user) {
                HxHelper.this.saveContact(user);
            }
        });
    }
     ...
     */
    public void init(Context context) {
        mSettingModel = new HxSettingModel(context);
...
//        options.setImPort(31097);
    ...
    private EMOptions initChatOptions(Context context) {
        ...
        options.setUseFCM(mSettingModel.isUseFCM());
        ...
        if (mSettingModel.isCustomServerEnable() && mSettingModel.getRestServer() != null && mSettingModel.getIMServer() != null) {
            options.setRestServer(mSettingModel.getRestServer());
            options.setIMServer(mSettingModel.getIMServer());
            if (mSettingModel.getIMServer().contains(":")) {
                options.setIMServer(mSettingModel.getIMServer().split(":")[0]);
                options.setImPort(Integer.valueOf(mSettingModel.getIMServer().split(":")[1]));
            }
        }

        if (mSettingModel.isCustomAppkeyEnabled() && mSettingModel.getCutomAppkey() != null && !mSettingModel.getCutomAppkey().isEmpty()) {
            options.setAppKey(mSettingModel.getCutomAppkey());
        }
    ...
    protected void setEaseUIProviders() {
        ...
        easeUI.setSettingsProvider(new EaseUI.EaseSettingsProvider() {
            ...
            public boolean isSpeakerOpened() {
                return mSettingModel.getSettingMsgSpeaker();
            }

            @Override
            public boolean isMsgVibrateAllowed(EMMessage message) {
                return mSettingModel.getSettingMsgVibrate();
            }

            @Override
            public boolean isMsgSoundAllowed(EMMessage message) {
                return mSettingModel.getSettingMsgSound();
            }

            @Override
            public boolean isMsgNotifyAllowed(EMMessage message) {
                if (message == null) {
                    return mSettingModel.getSettingMsgNotification();
                }
                if (!mSettingModel.getSettingMsgNotification()) {
                    return false;
                } else {
                    ...
                    if (message.getChatType() == ChatType.Chat) {
                        ...
                        notNotifyIds = mSettingModel.getDisabledIds();
                    } else {
                        chatUsename = message.getTo();
                        notNotifyIds = mSettingModel.getDisabledGroups();
                    }
    ...
    protected void setGlobalListeners() {
        ...
        isGroupsSyncedWithServer = mSettingModel.isGroupsSynced();
        isContactsSyncedWithServer = mSettingModel.isContactSynced();
        isBlackListSyncedWithServer = mSettingModel.isBacklistSynced();
...
//                }
    ...
    private boolean isDuringMediaCommunication() {
        ...
        return easeUI.hasForegroundActivies() && ("LiveActivity".equals(topClassName) || "ConferenceActivity".equals(topClassName));
    }
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            Log.d("firend", "MyContactListener-onContactAdded");
            // save contact
        ...
        public void onContactDeleted(String username) {
            Log.d("firend", "MyContactListener-delete");
        ...
        public void onContactInvited(String username, String reason) {
            Log.d("firend", "MyContactListener-invite");
            ...
            notifyNewInviteMessage(msg);
            broadcastManager.sendBroadcast(new Intent(Constant.ACTION_CONTACT_CHANAGED));
            showToast("onContactInvited:" + username);
        }

        @Override
        public void onFriendRequestAccepted(String username) {
            Log.d("firend", "MyContactListener-accept");
            List<InviteMessage> msgs = inviteMessgeDao.getMessagesList();
            for (InviteMessage inviteMessage : msgs) {
        ...
        public void onFriendRequestDeclined(String username) {
            ...
            Log.d("firend", "MyContactListener-declined");
            showToast("reclined:" + username);
        }
    }
...
//            return getUserProfileManager().getCurrentUserInfo();
     ...
     * @param callback
     */
    public void login(final String id, String password, final EMCallBack callback) {
        Log.d(TAG, "start hx login");

        if (TextUtils.isEmpty(id) || TextUtils.isEmpty(password)) {
            if (callback != null) {
                callback.onError(0, "username or password is null or empty!");
            }
            return;
        }
        ...
        EMClient.getInstance().login(id, password, new EMCallBack() {
            ...
            public void onSuccess() {
                Log.d(TAG, "hx login onSuccess");
                /*
                为了保证进入主页面后本地会话和群组都load完毕
                另外如果登录过,APP长期在后台再进的时候也可能会导致加载到内存的群组和会话为空,
...
//                EMClient.getInstance().chatManager().loadAllConversations();
            ...
            public void onError(int code, String error) {
                Log.d(TAG, "hx login onError = " + error);
                if (callback != null) {
            ...
            public void onProgress(int progress, String status) {
                Log.d(TAG, "hx login onProgress = " + progress);
                if (callback != null) {
    ...
    public HxSettingModel getModel() {
    ...
    public void saveContact(EaseUser user) {
        ...
        mSettingModel.saveContact(user);
    }
    ...
    public Map<String, EaseUser> getContactList() {
        if (isLoggedIn() && contactList == null) {
            contactList = mSettingModel.getContactList();
        }
    ...
    public void setCurrentUserName(String username) {
        ...
        mSettingModel.setCurrentUserName(username);
    }
    ...
    public String getCurrentUsernName() {
        if (username == null) {
            username = mSettingModel.getCurrentUsernName();
        }
        return username;
    }
    ...
    public Map<String, RobotUser> getRobotList() {
        if (isLoggedIn() && robotList == null) {
            robotList = mSettingModel.getRobotList();
        }
        return robotList;
    }
    ...
    public void updateContactList(List<EaseUser> contactInfoList) {
        ...
        List<EaseUser> mList = new ArrayList<>(contactList.values());
        mSettingModel.saveContactList(mList);
    }
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mSettingModel.setGroupsSynced(true);
                    ...
                    mSettingModel.setGroupsSynced(false);
                    isGroupsSyncedWithServer = false;
                    isSyncingGroupsWithServer = false;
                    noitifyGroupSyncListeners(false);
                    if (callback != null) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    mSettingModel.setContactSynced(true);
                    ...
                    mSettingModel.setContactSynced(false);
                    isContactsSyncedWithServer = false;
                    isSyncingContactsWithServer = false;
                    notifyContactsSyncListener(false);
                    e.printStackTrace();
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mSettingModel.setBlacklistSynced(true);
                    ...
                    mSettingModel.setBlacklistSynced(false);
