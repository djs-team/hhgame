...
- (void)_setupSubviews
{
    self.backgroundColor = UIColorHex(0xF6F6F6);
    ...
    self.textView.placeholder = @"请输入文字内容";
...
//    [self.buttonArray addObject:cameraButton];
    ...
    [giftButton setImage:[UIImage imageNamed:@"liwu"] forState:UIControlStateHighlighted];
    ...
    [self.buttonArray addObject:giftButton];
...
- (void)giftButtonAction:(UIButton *)aButton
{
    [self _buttonAction:aButton];
    if (aButton.selected) {
        if (self.giftView) {
            self.currentMoreView = self.giftView;
            [self addSubview:self.giftView];
            [self.giftView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(self);
                make.right.equalTo(self);
                make.bottom.equalTo(self).offset(-EMVIEWBOTTOMMARGIN);
                make.height.mas_equalTo(self.giftView.viewHeight);
            }];
            [self _remakeButtonsViewConstraints];
